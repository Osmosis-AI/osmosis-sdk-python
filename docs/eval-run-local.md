...
The run ends with a results table: work items by outcome, pass rate against the threshold, reward statistics, pass@k for multi-attempt runs, tokens, duration, and the workspace-relative output directory. Every number in it is read straight from what `metrics.json` holds — nothing is recomputed for display, so the terminal and the file cannot disagree. Durations use the same formatter as `osmosis eval info`, so a local run and a cloud run of the same evaluation read alike. A completed run that was not uploaded with `--upload` then prints a copyable `osmosis eval upload <run-name>` command. A run under a custom `--output` instead uses its workspace-relative or absolute directory, which `eval upload` continues to accept.
