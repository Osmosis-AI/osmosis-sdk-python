...
The run's provenance records the workspace-relative config path, so the platform can show the exact `--retry-failed` command on the run's detail page and `osmosis eval retry` can print it instead of refusing. Configs must live under the workspace's `configs/eval/` directory; other paths are rejected.
