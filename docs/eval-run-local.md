...
```bash
osmosis eval upload my-local-run
```
...
The run ends with a results table: work items by outcome, pass rate against the threshold, reward statistics, pass@k for multi-attempt runs, tokens, duration, and the output directory relative to where the command was invoked (or absolute when it lies outside that directory). Every number in it is read straight from what `metrics.json` holds — nothing is recomputed for display, so the terminal and the file cannot disagree. Durations use the same formatter as `osmosis eval info`, so a local run and a cloud run of the same evaluation read alike. A completed run that was not uploaded with `--upload` then prints a copyable `osmosis eval upload <run-name>` command. A run under a custom `--output` instead uses an invocation-relative or absolute directory, which `eval upload` continues to accept.
...
A work item is complete only when it has a durable terminal result. The supervisor writes and `fsync`s the journal record before treating the polled result as complete, so a `kill -9` cannot skip an uncommitted result. Ctrl-C cancels in-flight rollouts without writing a terminal record, so they simply run again next time. Cancellation uses a separate HTTP connection and waits up to 120 seconds for polled completion so workflow and sandbox cleanup can finish before server shutdown. Interrupted shutdown then allows up to 150 seconds for server exit, covering its initial drain, cancellation cleanup, final cancellation wait, and process-exit overhead. This also protects admission interrupted before a polling lease arrived, when cleanup must be initiated by server shutdown. Ordinary shutdown retains the five-second grace before force-stopping a server that does not exit. A second Ctrl-C exits immediately.

After admission, the supervisor bounds result polling by the sum of the configured agent and grader timeouts plus 60 seconds, treating zero or negative phase timeouts as zero budget. If either phase is unbounded, it uses a one-hour fallback. Reaching this deadline requests cancellation, halts dispatch, and leaves the unobserved work pending for resume instead of recording a row failure.

A `kill -9` normally does not leak the rollout server either. The supervisor records the server's process group, together with its pid and start time, in `server.json` at spawn, and removes the record on clean shutdown. A `--tunnel` run records its cloudflared child the same way in `tunnel.json` — an orphaned quick tunnel would otherwise keep a live public URL forwarding to a local port a later run may reuse — and the next invocation reaps both records under the same ownership proof. Orphaned rollout servers get the same 150-second cleanup window as interrupted runs; orphaned tunnels retain the five-second exit grace. The next invocation for the run — a plain resume, `--fresh`, even a refused resume — terminates the recorded group, but only after re-verifying that pid and start time still match: a record whose pid was recycled by an unrelated process is dropped, never signalled. Two narrow gaps remain, both resolved in favor of never signalling an unverified group: a kill that lands in the instant between the spawn and the record's write leaves no record, and a group whose recorded leader died while a descendant survived can no longer be verified, so it is dropped rather than killed blind. A server that slips through either gap still needs manual cleanup.
...
The run's provenance records the workspace-relative config path, so the platform can show the exact `--retry-failed` command on the run's detail page and `osmosis eval retry` can print it instead of refusing. Configs must live under the workspace's `configs/eval/` directory; other paths are rejected. `eval run --upload` sends the config it actually ran, which supersedes the manifest's when a resume follows a rename; a standalone `eval upload` has no invocation to read and sends the manifest's. The manifest itself is never rewritten, because its bytes are the `manifest_digest` that identifies the import.
...
The Harbor backend reports whether its resolved environment can reach host loopback. `eval run` leaves `LocalBackend`, Singularity, and macOS Docker on the direct path; it automatically starts cloudflared for model-calling cloud sandboxes and Linux Docker. Oracle's `wiring="none"` needs no model endpoint and therefore no tunnel. The terminal prints the detected environment before it opens ingress. `--tunnel cloudflared` remains a force override, and is also the fallback for an older rollout SDK or a third-party backend that does not report this capability.

An automatically selected or explicit `--tunnel cloudflared` starts a [Cloudflare quick tunnel](https://developers.cloudflare.com/cloudflare-one/networks/connectors/cloudflare-tunnel/do-more-with-tunnels/trycloudflare/) as a child of the run: it requires the `cloudflared` binary on `PATH` (macOS: `brew install cloudflared`), needs no account, prints the `https://<random>.trycloudflare.com` URL as a run milestone, is readiness-probed through the edge before any dispatch, and is terminated with the run. Model preflight, rollout dependency sync, and local rollout-server health all finish before cloudflared starts, so a local configuration or import failure does not consume the tunnel's startup wait or create a tunnel only to tear it down immediately. If the tunnel process dies mid-run — quick tunnels hold a single edge connection — dispatch halts with a clear error and the unfinished work items stay pending for a resume, exactly like a rollout-server death.
...
- **Corporate networks.** `*.trycloudflare.com` has abuse history and is blocked by some corporate egress filters, EDR products, and filtering DNS resolvers (Tailscale MagicDNS setups included). The readiness probe runs from your machine, whose DNS says nothing about the sandbox's: when the probe gets no HTTP response at all, the run continues with a warning that names the local failure category and lets the sandbox try. Only an edge that answers but keeps failing is treated as a dead tunnel. If the sandbox cannot reach it either, run your own tunnel and pass `--advertise-url`.
...
If a process-wide fault stops the run — the rollout server dies, the model provider rejects the key, or polling cannot read a result — dispatch halts and work items without a recorded terminal result remain **pending**, not marked failed. A 4xx refusal during admission is recorded as a row failure, but a 403 or 404 during result polling does not establish the rollout's outcome. Fix the cause and re-run the same `--name`; work without a recorded terminal result is dispatched again.

## Known limitations

- Harbor containers reach the bridge via `host.docker.internal`, which the loopback rewrite applies on macOS only. On Linux Docker the container cannot reach a loopback-bound bridge, so a model-calling rollout automatically uses cloudflared (or your `--advertise-url`) until a Linux gateway rewrite exists. `LocalBackend` rollouts work everywhere.
