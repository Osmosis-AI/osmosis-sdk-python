...
- Native OpenCode uses the supplied session endpoint through its OpenAI-compatible provider. Its API key stays in the agent environment. Auto-compaction and pruning are disabled to preserve the training token trajectory; other `native_agent_kwargs["opencode_config"]` settings, such as model context limits, are retained.
