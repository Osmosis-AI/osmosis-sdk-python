...
try:
    ...
    if OpenAI:
        ...
        if openai_api_key:
            openai_llm = OpenAI(
                model_name="gpt-3.5-turbo-instruct", openai_api_key=openai_api_key
            )
            try:
    ...
    if ChatAnthropic:
        ...
        if anthropic_api_key:
            chat_anthropic = ChatAnthropic(
                model="claude-2", anthropic_api_key=anthropic_api_key
            )
            try:
                ...
                anthropic_response = chat_anthropic.invoke(
                    [
                        HumanMessage(
                            content="Explain the concept of deep learning in simple terms."
                        )
                    ]
                )
    ...
    print(
        "The langchain-openai and langchain-anthropic adapters have captured model-specific details."
    )
