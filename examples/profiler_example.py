...
def compare_times(with_osmosis, without_osmosis):
    ...
    if diff > 0:
        print(
            f"  {GREEN}Osmosis-ai is faster by {abs(diff):.4f}s ({abs(percent):.2f}%){RESET}"
        )
    else:
        print(
            f"  {RED}Osmosis-ai is slower by {abs(diff):.4f}s ({abs(percent):.2f}%){RESET}"
        )
    ...
    def run_langchain_anthropic_without():
        try:
            ...
            chat_anthropic = ChatAnthropic(
                model="claude-3-haiku-20240307", anthropic_api_key=api_key
            )
...
if __name__ == "__main__":
    ...
    print(
        f"This script compares execution times for different integrations with and without osmosis-ai."
    )
        ...
        for integration, times in results.items():
            if times:
                ...
                if diff > 0:
                    ...
                    performance = (
                        f"{RED}SLOWER by {abs(diff):.4f}s ({abs(percent):.2f}%){RESET}"
                    )

                print(
                    f"{BOLD}{integration.upper()}{RESET}: Osmosis-ai is {performance}"
                )
