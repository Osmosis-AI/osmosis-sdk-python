...
class HarborBackendV2(ExecutionBackend):
    def __init__(
        ...
        *,
        ...
        if agent_setup_timeout_sec is not None and not (
            ...
            # Harbor feeds this straight into asyncio.wait_for: 0/negative
            # fails every trial instantly, NaN is undefined.
        ...
        # Deep-copy before applying backend-managed placement: the placement
        # helper mutates kwargs in place, and later caller mutations must not
        # leak into the per-trial copies.
        ...
        if max_queue_depth is not None and max_queue_depth < 1:
            # The backend cannot see the orchestrator's concurrency, so a
            # depth of 0 ("slots but no waiters") is unenforceable here; it
            # previously meant reject-everything.
            raise ValueError("max_queue_depth must be >= 1, or None for unbounded")
        self.max_queue_depth = max_queue_depth
        self.pending: dict[str, PendingTrial] = {}
        # Trial names generated by prewarm_trial_config; hooks dispatch on
        # membership, never on patterns in caller-controlled rollout ids.
    ...
    def build_agent_config(
        ...
        if self.native is not None and isinstance(self.agent, str):
            ...
            # Read by key presence: a present-but-invalid override must fail,
            ...
            if self.native.wiring != "none" and not url:
                # An empty api_base makes litellm fall back to the provider's
                # public endpoint, sending the rollout credential off-host.
                raise ValueError(
    ...
    def materialize_task(
        ...
        return task.materialize(
            ...
            # Only the bundled harness reads the top-level input file (native
            # graders get their own copy in tests/); skip staging the api_key
            # on tracks with no consumer.
    ...
    async def execute(
        ...
        if request.id in self.pending:
            ...
            # rewrite its staged input files (credential and training-data
            # crossover), then strand one of the two execute() calls forever.
            raise ValueError(
        ...
        try:
            ...
            # Instruction ownership: dataset mode zeroes the prompt, so the
            # task always keeps its own instruction.md. In template mode the
            # prompt owns the instruction — silently for the configured
            # template dir (its instruction.md is a fallback by design), with
            # a warning for any other task, however it was selected, whose
            # authored instruction.md is about to be replaced.
            if (
    ...
    def archive_trial(
        ...
        secrets from the trial directory before ``submit()`` returns, so
        relocating from here is the only ordering under which nothing reaches
        durable storage unredacted. Trial-hook code must never relocate.
        """
    ...
    def cleanup_rollout_residue(self, rollout_id: str, *, include_trial: bool) -> None:
        ...
        The staging dir can hold the rollout api_key in cleartext; cancelled
        and setup-failed rollouts must not leave it behind. Failed trials keep
        ...
        try:
            ...
            for config, outcome in zip(configs, outcomes, strict=True):
                ...
                if isinstance(outcome, BaseException):
                    ...
                    # failures keep their directories for debugging.
    ...
    def prewarm_agent_config(self, task_dir: Path) -> HarborAgentConfig:
        """Install-only trials run agent setup but never the agent, so their
        config carries no endpoint and no credentials."""
        if self.native is not None and isinstance(self.agent, str):
    ...
    async def on_trial_end(self, event: TrialHookEvent) -> None:
        ...
        if event.config.trial_name in self.prewarm_trials:
            ...
            # submit() resolves (post-scrub); doing it here would act on a
            # pattern in a caller-controlled id and pre-empt harbor's scrub.
            return
        rollout_id = parse_rollout_id(event)
