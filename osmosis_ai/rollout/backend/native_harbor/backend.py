...
class NativeHarborBackend(ExecutionBackend):
    ...
    def _build_grader_result(
        ...
        The rollout server only forwards ``samples`` on the grader callback (the
        workflow callback carries status only), and the controller correlates the
        callback to the rollout by id, so keying the sole sample on ``request.id``
        is the natural single-sample mapping onto the multi-sample grader contract.
        """
