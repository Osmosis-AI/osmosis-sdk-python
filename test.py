"""
Tests for osmosis-ai functionality

This file tests the monkey patching functionality of osmosis-ai for various LLM APIs.
"""

from osmosis_ai.consts import LogDestination
import pytest
from unittest.mock import patch, MagicMock

# Import osmosis_ai and set up test environment
import osmosis_ai
...
# Initialize osmosis_ai with a test API key
...
def setup_osmosis():
    ...
    osmosis_ai.init("test-osmosis-api-key")

    # Patch all possible references to send_to_osmosis
    original_send_to_osmosis = osmosis_ai.utils.send_to_osmosis

    # Replace the function with our mock
    osmosis_ai.utils.send_to_osmosis = mock_send_to_osmosis

    # Also patch it in the adapters
    try:
        import osmosis_ai.adapters.anthropic

        osmosis_ai.adapters.anthropic.send_to_osmosis = mock_send_to_osmosis
    except ImportError:
        pass

    try:
        import osmosis_ai.adapters.openai

        osmosis_ai.adapters.openai.send_to_osmosis = mock_send_to_osmosis
    except ImportError:
        pass
    ...
    osmosis_ai.utils.send_to_osmosis = original_send_to_osmosis
...
def test_anthropic_aiping(setup_osmosis):
    ...
    mock_response.model_dump = MagicMock(
        return_value={"content": [{"text": "Mocked Anthropic Response"}]}
    )
    ...
    try:
        ...
        from osmosis_ai.adapters.anthropic import wrap_anthropic
        ...
        assert (
            mock_send_to_osmosis.call_args[1]["query"]["model"]
...
def test_anthropic_async_aiping(setup_osmosis):
    mock_send_to_osmosis = setup_osmosis
    mock_send_to_osmosis.reset_mock()  # Start with a clean mock
    ...
    mock_response.model_dump = MagicMock(
        return_value={"content": [{"text": "Mocked Async Anthropic Response"}]}
    )
...
def test_anthropic_tool_use(setup_osmosis):
    ...
    mock_response.model_dump = MagicMock(
        return_value={"content": [{"type": "tool_use", "tool_use": tool_call}]}
    )
...
def test_anthropic_tool_response(setup_osmosis):
    ...
    first_response.model_dump = MagicMock(
        return_value={"content": [{"type": "tool_use", "tool_use": tool_call}]}
    )
    ...
    second_response.model_dump = MagicMock(
        return_value={
            "content": [
                {
                    "type": "text",
    ...
    try:
        ...
        tool_response = {
            "type": "tool_response",
            "tool_call_id": "tu_01",
            "content": {"temperature": 65, "condition": "sunny", "humidity": 70},
        }
...
def test_openai_v1_aiping(setup_osmosis):
    mock_send_to_osmosis = setup_osmosis
    ...
    mock_response.model_dump = MagicMock(
        return_value={"choices": [{"message": {"content": "Mocked OpenAI Response"}}]}
    )
    ...
    try:
        ...
        # Force re-aiping
        from osmosis_ai.adapters.openai import wrap_openai
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            max_tokens=150,
            messages=[{"role": "user", "content": "Test prompt for OpenAI v1"}],
        )
...
def test_openai_v1_async_aiping(setup_osmosis):
    mock_send_to_osmosis = setup_osmosis
    mock_send_to_osmosis.reset_mock()  # Reset call count
    ...
    try:
        ...
        from osmosis_ai.adapters.openai import wrap_openai, _ai_openai_v1
        ...
        mock_response = MagicMock()
        mock_response.choices = [
            MagicMock(message=MagicMock(content="Mocked Async OpenAI Response"))
        ]
        mock_response.model_dump = MagicMock(
            return_value={
                "choices": [{"message": {"content": "Mocked Async OpenAI Response"}}]
            }
        )
        ...
        async def simulate_async_call():
            ...
            send_to_osmosis(
                query=query_params, response=mock_response.model_dump(), status=200
            )
            return mock_response
        ...
        for name, method in inspect.getmembers(completions.Completions):
            if (
                ...
                assert hasattr(
                    method, "_osmosis_aiped"
                ), f"Async method {name} was not wrapped"
...
def test_openai_v2_aiping(setup_osmosis):
    mock_send_to_osmosis = setup_osmosis
    mock_send_to_osmosis.reset_mock()  # Reset call count
    ...
    try:
        ...
        mock_response = MagicMock()
        mock_response.choices = [
            MagicMock(message=MagicMock(content="Mocked OpenAI v2 Response"))
        ]
        mock_response.model_dump = MagicMock(
            return_value={
                "choices": [{"message": {"content": "Mocked OpenAI v2 Response"}}]
            }
        )
        ...
        with patch.object(
            ...
            response = client.chat.completions.create(
                model="gpt-4o-mini",
                max_tokens=150,
                messages=[{"role": "user", "content": "Test prompt for OpenAI v2"}],
            )
...
def test_openai_v2_async_aiping(setup_osmosis):
    mock_send_to_osmosis = setup_osmosis
    mock_send_to_osmosis.reset_mock()  # Reset call count
    ...
    try:
        ...
        # Import the OpenAI adapter and apply wrapping
        from osmosis_ai.adapters.openai import wrap_openai, _ai_openai_v2
        ...
        mock_response = MagicMock()
        mock_response.choices = [
            MagicMock(message=MagicMock(content="Mocked Async OpenAI v2 Response"))
        ]
        mock_response.model_dump = MagicMock(
            return_value={
                "choices": [{"message": {"content": "Mocked Async OpenAI v2 Response"}}]
            }
        )
        ...
        assert hasattr(
            AsyncOpenAI.__init__, "_osmosis_aiped"
        ), "AsyncOpenAI.__init__ was not wrapped"
...
def test_disable_osmosis(setup_osmosis):
    ...
    osmosis_ai.disable_osmosis()
    assert not osmosis_ai.utils.enabled
    ...
    with patch("anthropic.Anthropic") as MockAnthropic:
        ...
        try:
            ...
            with patch("openai.OpenAI") as MockOpenAI:
                mock_response = MagicMock()
                mock_response.choices = [
                    MagicMock(message=MagicMock(content="Response while disabled"))
                ]
                ...
                client.chat.completions.create(
                    ...
                    max_tokens=150,
                    messages=[
                        {"role": "user", "content": "This request won't be logged."}
                    ],
                )
    ...
    # Re-enable and test
    osmosis_ai.enable_osmosis()
    assert osmosis_ai.utils.enabled
...
def test_openai_async_support(setup_osmosis):
    """Test that osmosis_ai supports async OpenAI clients by directly checking the adapter code."""
    mock_send_to_osmosis = setup_osmosis
    mock_send_to_osmosis.reset_mock()  # Reset call count
    ...
    try:
        ...
        from osmosis_ai.adapters import openai as openai_adapter

        # Check for _ai_openai_v1 and _ai_openai_v2 functions
        assert hasattr(
            openai_adapter, "_ai_openai_v1"
        ), "Missing _ai_openai_v1 function"
        assert hasattr(
            openai_adapter, "_ai_openai_v2"
        ), "Missing _ai_openai_v2 function"

        # Check for async wrapping in _ai_openai_v1 function
        v1_aiper_code = inspect.getsource(openai_adapter._ai_openai_v1)
        assert (
            "async def wrapped_async_method" in v1_aiper_code
        ), "V1 wrapper missing async support"

        # Check for async wrapping in _ai_openai_v2 function
        v2_aiper_code = inspect.getsource(openai_adapter._ai_openai_v2)
        assert (
            "async def wrapped_achat_create" in v2_aiper_code
        ), "V2 wrapper missing async chat support"
        assert (
            "async def wrapped_acompletions_create" in v2_aiper_code
        ), "V2 wrapper missing async completions support"

        # Check for AsyncOpenAI specific wrapping
        assert (
            "AsyncOpenAI" in v1_aiper_code or "AsyncOpenAI" in v2_aiper_code
        ), "No direct AsyncOpenAI support found"
    ...
    second_response.model_dump = MagicMock(
        return_value={
            "content": [
                {
                    "type": "text",
                    "text": "I found several wireless headphones in the electronics category. The top options are Sony WH-1000XM5, Apple AirPods Pro, and Bose QuietComfort.",
                }
            ]
        }
    )
...
def test_langchain_llm_aiping(setup_osmosis):
    mock_send_to_osmosis = setup_osmosis
    mock_send_to_osmosis.reset_mock()  # Start with a clean mock
    ...
    try:
        ...
        # Force the wrapping to occur
        from osmosis_ai.adapters.langchain import wrap_langchain
...
def test_langchain_chat_model_aiping(setup_osmosis):
    mock_send_to_osmosis = setup_osmosis
    mock_send_to_osmosis.reset_mock()  # Start with a clean mock
...
def test_langchain_prompt_template_aiping(setup_osmosis):
    mock_send_to_osmosis = setup_osmosis
    mock_send_to_osmosis.reset_mock()  # Start with a clean mock
    ...
    # Force the wrapping to occur
    from osmosis_ai.adapters.langchain import wrap_langchain, _patch_langchain_prompts
...
def test_langchain_async_llm_aiping(setup_osmosis):
    mock_send_to_osmosis = setup_osmosis
    mock_send_to_osmosis.reset_mock()  # Start with a clean mock
...
@pytest.mark.skipif(
    pytest.importorskip(
        "langchain_anthropic", reason="langchain-anthropic package not installed"
    )
    is None,
    reason="LangChain-Anthropic package not installed",
)
def test_langchain_anthropic_aiping(setup_osmosis):
    mock_send_to_osmosis = setup_osmosis
    mock_send_to_osmosis.reset_mock()  # Start with a clean mock
    ...
    try:
        ...
        if hasattr(ChatAnthropic, "_generate"):
            ...
            def mock_generate(*args, **kwargs):
                ...
                return LLMResult(
                    generations=[[Generation(text="Mocked ChatAnthropic response")]]
                )
...
@pytest.mark.skipif(
    pytest.importorskip(
        "langchain_openai", reason="langchain-openai package not installed"
    )
    is None,
    reason="LangChain-OpenAI package not installed",
)
def test_langchain_openai_aiping(setup_osmosis):
    mock_send_to_osmosis = setup_osmosis
    mock_send_to_osmosis.reset_mock()  # Start with a clean mock
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                pytest.skip(
                    "Could not find OpenAI or ChatOpenAI in any expected location"
                )
        ...
        if hasattr(ChatOpenAI, "_generate"):
            ...
            def mock_generate(*args, **kwargs):
                ...
                return LLMResult(
                    generations=[[Generation(text="Mocked ChatOpenAI response")]]
                )
        ...
        utils.send_to_osmosis(
            query=query_data_llm, response=response_data_llm, status=200
        )
        ...
        utils.send_to_osmosis(
            query=query_data_chat, response=response_data_chat, status=200
        )
...
def test_langchain_azure_openai_aiping(setup_osmosis):
    mock_send_to_osmosis = setup_osmosis
    mock_send_to_osmosis.reset_mock()  # Start with a clean mock
    ...
    try:
        ...
        try:
            ...
            print(
                "Successfully imported AzureOpenAI and AzureChatOpenAI from langchain_openai"
            )
        except ImportError:
            try:
                ...
                pytest.skip(
                    "Could not find AzureOpenAI or AzureChatOpenAI in any expected location"
                )
        ...
        if hasattr(AzureChatOpenAI, "_generate"):
            ...
            def mock_azure_generate(*args, **kwargs):
                ...
                return LLMResult(
                    generations=[[Generation(text="Mocked AzureChatOpenAI response")]]
                )
        ...
        utils.send_to_osmosis(
            query=query_data_azure_llm, response=response_data_azure_llm, status=200
        )
        ...
        utils.send_to_osmosis(
            query=query_data_azure_chat, response=response_data_azure_chat, status=200
        )
