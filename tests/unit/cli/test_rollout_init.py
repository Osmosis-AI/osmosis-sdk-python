...
def test_rollout_init_main_py_is_a_runnable_rollout_server(
    ...
    assert "osmosis_ai.rollout.integrations.openai_agents" in main_py
    assert "osmosis_ai.rollout.integrations.agents" not in main_py
    pyproject_toml = (
        workspace_directory / "rollouts" / "my-agent" / "pyproject.toml"
    ).read_text(encoding="utf-8")
    assert '"osmosis-ai[server]>=0.3.0rc1"' in pyproject_toml
