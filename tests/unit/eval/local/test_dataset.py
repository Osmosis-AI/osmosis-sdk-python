...
def test_a_cache_entry_with_no_recorded_size_is_a_miss(tmp_path: Path) -> None:
    # file_size is the cache's only post-store integrity check; an entry
    # missing it cannot be validated, so it re-downloads rather than serving
    # bytes no check ever vouched for.
    cache = DatasetCache(tmp_path / "cache")
    fetcher = FakeFetcher(rows=PROMPT_ROWS)
    resolved = resolve_platform_dataset("multiply", cache=cache, fetcher=fetcher)
