...
def test_submit_training_forwards_provided_secrets() -> None:
    client = MagicMock()
    train_mod._submit_training(
        client,
        _train_config(["GITHUB_TOKEN"]),
        credentials=None,
        git_identity="g",
        provided_secrets={"GITHUB_TOKEN": "supplied"},
    )
    kwargs = client.submit_training_run.call_args.kwargs
    assert kwargs["provided_secrets"] == {"GITHUB_TOKEN": "supplied"}


def test_submit_eval_forwards_provided_secrets() -> None:
    client = MagicMock()
    eval_mod._submit_eval(
        client,
        _eval_config(["OPENAI_API_KEY"]),
        credentials=None,
        git_identity="g",
        provided_secrets={"OPENAI_API_KEY": "supplied"},
    )
    kwargs = client.submit_evaluation_run.call_args.kwargs
    assert kwargs["provided_secrets"] == {"OPENAI_API_KEY": "supplied"}


def test_submit_passes_none_when_no_provided_secrets() -> None:
    client = MagicMock()
    eval_mod._submit_eval(
        client,
        _eval_config(["OPENAI_API_KEY"]),
    ...
    assert client.submit_evaluation_run.call_args.kwargs["provided_secrets"] is None
